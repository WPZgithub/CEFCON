...
if(length(args)==4) end_cluster <- args[4]
...
exp_data <- t(exp_data)
reducedDim_data <- read.csv(paste0(out_dir, '/data_reducedDim.csv'), header = TRUE)
reducedDim_data <- as.matrix(reducedDim_data)
cluster_labels <- read.csv(paste0(out_dir, '/clusters.csv'), header = TRUE, row.names = 1)

## Slingshot
if(exists('end_cluster')){
  sds <- slingshot(reducedDim_data, cluster_labels[[1]], start.clus=start_cluster, end.clus=end_cluster)
} else {
  sds <- slingshot(reducedDim_data, cluster_labels[[1]], start.clus=start_cluster)
}
pseudotime <- slingPseudotime(sds)
colnames(pseudotime) <- apply(pseudotime, 2,function(x) paste0(start_cluster,'to',cluster_labels[[1]][which.max(x)]))
row.names(pseudotime) <- row.names(cluster_labels)
write.csv(pseudotime, paste0(out_dir, '/pseudotime_lineages.csv'), quote=FALSE)
...
{
  lin <- pseudotime[, p]
  names(lin) <- row.names(pseudotime)
  lin <- lin[!is.na(lin)]
